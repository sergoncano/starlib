...
pub fn get_rendering_hashmap(list: Vec<Box<dyn Renderable>>) -> HashMap<Coords, Box<dyn Renderable>> {
        ...
        if !map.contains_key(&coords) {
            ...
            if current_item.get_z_index() < renderable_item.get_z_index() {
                *map.get_mut(&coords).unwrap() = renderable_item;
                ...
                panic!("Z-fighting between objects {:?} and {:?}", stringify!(current_item), stringify!(renderable_item));
            }
        }
    }
    map
